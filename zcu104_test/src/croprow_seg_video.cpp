...
static bool process_frame(
    ...
    const float CONF_THRES = 0.1f;
    const float NMS_THRES = 0.45f;
    const int NUM_MASKS = 32;
    const float MASK_THRES = 0.35f;

    // Với model crop-row NCNN hiện tại, class crop-row đang ra class 0 sau khi ép task=segment.
    // Để tự chọn class score cao nhất, đổi thành -1.
    ...
    cv::TickMeter infer_timer;
    infer_timer.start();

    if (ex.input("in0", in) != 0) {
        std::cerr << "Failed to set input blob: in0\n";
        return false;
    }
    ...
    if (frame_id % 10 == 0) {
        std::cout << "NCNN inference only frame=" << frame_id
                  << " time=" << infer_timer.getTimeMilli()
                  << " ms" << std::endl;
    }
    ...
    if (num_classes <= 0) {
        std::cerr << "Invalid num_classes. Check out0 shape.\n";
        return false;
    }
    ...
    for (int idx = 0; idx < (int)keep.size(); ++idx) {
        ...
        total_nonzero += full_nonzero;

        // Hiển thị full mask để nhìn rõ luồng cây.
        ...
        // Chỉ blend trong vùng mask, không làm tối toàn ảnh.
        cv::addWeighted(overlay, 0.70, color_layer, 0.30, 0, blended);
        ...
        if (save_masks && saved_mask_count < 3) {
            char full_mask_png[256];
            char cropped_mask_png[256];
            char mask_bin_path[256];
            char info_path[256];

            snprintf(full_mask_png, sizeof(full_mask_png),
                     "output/video_mask_full_f%05d_obj%d.png", frame_id, idx);

            snprintf(cropped_mask_png, sizeof(cropped_mask_png),
                     "output/video_mask_cropped_f%05d_obj%d.png", frame_id, idx);

            snprintf(mask_bin_path, sizeof(mask_bin_path),
                     "output/video_mask_full_f%05d_obj%d.bin", frame_id, idx);

            snprintf(info_path, sizeof(info_path),
                     "output/video_mask_f%05d_obj%d_info.txt", frame_id, idx);
    ...
    if (frame_id % 10 == 0) {
        std::cout << "frame=" << frame_id
                  ...
                  << ", objects=" << keep.size()
                  << ", mask_nonzero_total=" << total_nonzero << std::endl;
    }
...
int main(int argc, char** argv) {
    ...
    std::cout << "Size: " << width << "x" << height << ", fps=" << fps << std::endl;
    ...
    if (!writer_ok) {
        std::cerr << "Warning: Cannot open VideoWriter. Saving frames as JPG instead.\n";
    }

    int frame_id = 0;
    int save_mask_every = 10;
    ...
    while (true) {
        ...
        if (!ok) {
            std::cerr << "Failed at frame " << frame_id << std::endl;
            break;
        }
        ...
        if (max_frames > 0 && frame_id >= max_frames) {
            std::cout << "Reached max_frames=" << max_frames << std::endl;
            break;
        }
    }
    ...
    std::cout << "Done.\n";
    std::cout << "Processed frames: " << frame_id << std::endl;
    std::cout << "Total time: " << total_sec << " sec" << std::endl;
    std::cout << "Average FPS: " << avg_fps << std::endl;
    std::cout << "Saved: output/result_video.avi" << std::endl;
